...
public class FFAction3 extends JPanel {
    ...
    public FFAction3(GUI gui, ServerInterface server, JFrame parent, int game, String nickName) throws RemoteException {
        ...
        add(new JLabel("Final Frenzy action 3.\n" +
                "You may move up to 2 squares and grab something."));
    ...
    private void finalGrab() {
        try {
            if(!this.server.messageIsValidFinalFrenzyAction3(game, nickName, directions, wCard, weaponSlot, lC, lP, lPC)) {
                gui.thirdFFAction(false);
            }
            server.messageFinalFrenzyAction3(game, nickName, directions, wCard, lC, lP, lPC);
