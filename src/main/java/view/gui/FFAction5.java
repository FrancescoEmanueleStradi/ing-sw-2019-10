...
public class FFAction5 extends JPanel {
    ...
    public FFAction5(GUI gui, ServerInterface server, JFrame parent, int game, String nickName, Timer timer) throws RemoteException {
        super();
        this.gui = gui;
        this.server = server;
        this.parent = parent;
    ...
    private void finalGrab() {
        try {
            if(!this.server.messageIsValidFinalFrenzyAction5(game, nickName, directions, wCard, weaponSlot, lC, lP, lPC)) {
                gui.fifthFFAction(false);
            }
            server.messageFinalFrenzyAction5(game, nickName, directions, wCard, lC, lP, lPC);
